...
export function saveNewStep(stepRequest) {
  return request({
    url:
      API_BASE_URL +
      ('/step' + (stepRequest.id !== undefined ? '/' + stepRequest.id : '')),
...
export function stepsFetchAllWithForeignData() {
  return request({
    url: API_BASE_URL + '/step/getAllSteps',
    method: 'GET',
  });
}
...
export function saveNewRecitationTemplate(recitationTemplateRequest) {
  return request({
    url:
      API_BASE_URL +
      ('/recitationtemplate' +
        (recitationTemplateRequest.id !== undefined
          ? '/' + recitationTemplateRequest.id
          : '')),
...
export function saveNewRecitationCenter(recitationCenterRequest) {
  return request({
    url:
      API_BASE_URL +
      ('/center' +
        (recitationCenterRequest.id !== undefined
          ? '/' + recitationCenterRequest.id
          : '')),
...
export function getAllStudentProgress() {
  return request({
    url: API_BASE_URL + '/studentprogress',
    method: 'GET',
  });
}

export function getAllStudentProgressForUserByStepId(userId, stepId) {
  return request({
    url:
      API_BASE_URL +
      '/studentprogress/findByUserAndStepId/' +
      userId +
      '/' +
      stepId,
    method: 'GET',
...
export function saveStudentProgress(studentprogressRequest) {
  return request({
    url: API_BASE_URL + '/studentprogress',
    method: 'POST',
    body: JSON.stringify(studentprogressRequest),
  });
}
export function updateStudentProgress(studentprogressRequest) {
  return request({
    url: API_BASE_URL + '/studentprogress/' + studentprogressRequest.id,
    method: 'PUT',
    body: JSON.stringify(studentprogressRequest),
  });
}
export function deleteStudentProgress(id) {
  return request({
    url: API_BASE_URL + '/studentprogress/' + parseInt(id),
    method: 'DELETE',
  });
}
...
export function courseWhiteboardsByStudentStructure(
  courseId,
  page,
  docId,
  type,
